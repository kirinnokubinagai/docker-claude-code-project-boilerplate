...
if [ "$1" = "--dynamic" ]; then
    ...
    # 動的チーム構成用のプロンプトファイルを作成
    cat > "$WORKSPACE/DYNAMIC_TEAM_PROMPT.md" << 'EOF'
# 動的チーム構成の実行

CLAUDE_MASTER_DYNAMIC.mdを読んで、プロジェクトに最適なチーム構成を決定してください。

## 手順

1. **プロジェクト要件の収集**
   - ユーザーにプロジェクトの詳細を質問してください
   - どのようなアプリケーションを作りたいか
   - 想定される機能や規模
   - 技術的な要望があるか
...
4. **config/teams.jsonの生成**
   ```json
   {
     "project_name": "プロジェクト名",
     "project_type": "タイプ",
     "analyzed_at": "ISO形式の日時",
     "teams": [
       {
         "id": "チームID",
         "name": "チーム名",
...
プロジェクトの成功に必要な最適なチーム構成を作成してください。
EOF
    
    log_success "動的チーム構成の準備が完了しました"
    echo ""
    echo "📝 Claude Codeを起動して、プロジェクト要件を入力してください"
    echo ""
    echo "例:"
    ...
    echo "  claude code"
    echo ""
    echo "起動後、DYNAMIC_TEAM_PROMPT.mdの内容に従ってチーム構成を決定してください"
    exit 0
fi
